...
/*void write_Root(Root* restrict root, char* filename){
    ...
    ASSERT_NULL_PTR(root->info_per_lvl,"write_Root()")
    ASSERT_NULL_PTR(root->hash_v,"write_Root()")
    ...
    write_Node(&(root->node), root, (root->k / NB_CHAR_SUF_PREF) - 1, file, root->k);
...
void write_Node(Node* restrict node, Root* root, int lvl_node, FILE* file, int size_kmer){
    ...
    if ((CC*)node->CC_array != NULL){
        do {
            ...
            write_CC(&(((CC*)node->CC_array)[i]), root, lvl_node, file, size_kmer);
        }
    ...
    fwrite(&flags, sizeof(uint8_t), 1, file);
    write_UC(&(node->UC_array), file, root->info_per_lvl[(size_kmer/NB_CHAR_SUF_PREF)-1].size_kmer_in_bytes, node->UC_array.nb_children >> 1);
...
void write_CC(CC* restrict cc, Root* root, int lvl_cc, FILE* file, int size_kmer){
    ...
    fwrite(cc->BF_filter2, sizeof(uint8_t), size_bf + (MASK_POWER_16[p]/SIZE_BITS_UINT_8T), file);
    ...
    if (size_kmer != NB_CHAR_SUF_PREF){
        ...
        for (i = 0; i < nb_skp; i++){
            ...
            uc = &(((UC*)cc->children)[i]);
            write_UC(uc, file, root->info_per_lvl[lvl_cc].size_kmer_in_bytes_minus_1, uc->nb_children);
        }
    }
    else{
        for (i = 0; i < nb_skp; i++){

            if (i != nb_skp-1) write_UC(&(((UC*)cc->children)[i]), file, 0, root->info_per_lvl[lvl_cc].nb_ucs_skp);
            else write_UC(&(((UC*)cc->children)[i]), file, 0, cc->nb_elem - i * root->info_per_lvl[lvl_cc].nb_ucs_skp);
        }
...
Root* read_Root(char* filename){
    ...
    if (fread(&r1, sizeof(int), 1, file) != 1) ERROR("read_Root()")
    if (fread(&r2, sizeof(int), 1, file) != 1) ERROR("read_Root()")

    Root* root = createRoot(NULL, 0, 0, 0, 0, r1, r2, NULL);
    ...
    if (fread(&(root->compressed), sizeof(uint8_t), 1, file) != 1) ERROR("read_Root()")
    ...
    if (root->length_comp_set_colors != 0){
        ...
        for (i=0; i<root->length_comp_set_colors; i++){
            ...
            if (fread(&(root->comp_set_colors[i].size_annot), sizeof(int), 1, file) != 1) ERROR("read_Root()")
    ...
    for (i = 0; i < root->k / NB_CHAR_SUF_PREF; i++){
        if (fread(&(root->info_per_lvl[i].nb_bits_per_cell_skip_filter2), sizeof(int), 1, file) != 1) ERROR("read_Root()")
        if (fread(&(root->info_per_lvl[i].nb_bits_per_cell_skip_filter3), sizeof(int), 1, file) != 1) ERROR("read_Root()")
        if (fread(&(root->info_per_lvl[i].nb_ucs_skp), sizeof(int), 1, file) != 1) ERROR("read_Root()")
        if (fread(&(root->info_per_lvl[i].nb_kmers_uc), sizeof(int), 1, file) != 1) ERROR("read_Root()")
        if (fread(&(root->info_per_lvl[i].level_min), sizeof(int), 1, file) != 1) ERROR("read_Root()")
        if (fread(&(root->info_per_lvl[i].modulo_hash), sizeof(int), 1, file) != 1) ERROR("read_Root()")
        if (fread(&(root->info_per_lvl[i].tresh_suf_pref), sizeof(int), 1, file) != 1) ERROR("read_Root()")
...
void read_Node(Node* restrict node, Root* root, int lvl_node, FILE* file, int size_kmer){
    ...
    if ((flags & CC_VERTEX) != 0){
        while ((flags & CC_VERTEX) != 0){
            ...
            read_CC(&(((CC*)node->CC_array)[i]), root, lvl_node, file, size_kmer);
            ...
            if ((((CC*)node->CC_array)[i].type & 0x1) == 1){
                ...
                if ((flags & UC_PRESENT) != 0){
                    ...
                    read_UC(&(node->UC_array), file, root->info_per_lvl[(size_kmer/NB_CHAR_SUF_PREF)-1].size_kmer_in_bytes, nb_children >> 1);
...
void read_CC(CC* restrict cc, Root* root, int lvl_cc, FILE* file, int size_kmer){
    ...
    ASSERT_NULL_PTR(cc,"read_CC()")
    ASSERT_NULL_PTR(root, "read_CC()")
    ...
    cc->BF_filter2 = malloc((bf_filter2 + skipFilter2 + skipFilter3) * sizeof(uint8_t));
    ...
    if (size_kmer != NB_CHAR_SUF_PREF){
        ...
        for (i = 0; i < nb_skp; i++){
            ...
            uc = &(((UC*)cc->children)[i]);
            read_UC(uc, file, root->info_per_lvl[lvl_cc].size_kmer_in_bytes_minus_1, nb_children);
        ...
        for (i = 0; i < nb_skp; i++){
            ...
            if (i != nb_skp-1) read_UC(&(((UC*)cc->children)[i]), file, 0, root->info_per_lvl[lvl_cc].nb_ucs_skp);
            else read_UC(&(((UC*)cc->children)[i]), file, 0, cc->nb_elem - i * root->info_per_lvl[lvl_cc].nb_ucs_skp);
    ...
    if (root->info_per_lvl[lvl_cc].level_min == 1){
        ...
        for (i = 0; i < skipFilter3 * root->info_per_lvl[lvl_cc].nb_bytes_per_cell_skip_filter3; i += root->info_per_lvl[lvl_cc].nb_bytes_per_cell_skip_filter3){
        ...
        type = cc->children_type[0];

        for (i=0; i<skipFilter3 * root->info_per_lvl[lvl_cc].nb_bits_per_cell_skip_filter3; i++){

            if (i%root->info_per_lvl[lvl_cc].nb_ucs_skp == 0){
                pos_uc++;
                it_children = 0;
            ...
            if ((nb_elt = getNbElts(cc, i, type)) == 0){
                count_1 += cc->children_Node_container[it_nodes].UC_array.nb_children & 0x1;
                it_nodes++;
            }
            else {
                count_1 += uc->suffixes[it_children * size_line + root->info_per_lvl[lvl_cc].size_kmer_in_bytes_minus_1-1] >> 7;
                it_children += nb_elt;
            }

            if (i%root->info_per_lvl[lvl_cc].nb_bits_per_cell_skip_filter3 == root->info_per_lvl[lvl_cc].nb_bits_per_cell_skip_filter3-1){
...
void write_Node_sparse(Node* restrict node, Root* root, int lvl_node, FILE* file, int size_kmer){
    ...
    if ((CC*)node->CC_array != NULL){
        do {
            ...
            write_CC_sparse(&(((CC*)node->CC_array)[i]), root, lvl_node, file, size_kmer);
        }
        while ((((CC*)node->CC_array)[i].type & 0x1) == 0);
    }
    ...
    flags = UC_VERTEX | UC_PRESENT;
    fwrite(&flags, sizeof(uint8_t), 1, file);
    write_UC_sparse(&(node->UC_array), root, file, root->info_per_lvl[(size_kmer/NB_CHAR_SUF_PREF)-1].size_kmer_in_bytes, node->UC_array.nb_children >> 1);
...
void read_Node_sparse(Node* restrict node, Root* root, int lvl_node, FILE* file, int size_kmer){
    ...
    if ((flags & CC_VERTEX) != 0){
        while ((flags & CC_VERTEX) != 0){
            ...
            read_CC_sparse(&(((CC*)node->CC_array)[i]), root, lvl_node, file, size_kmer);
            ...
            if ((((CC*)node->CC_array)[i].type & 0x1) == 1){
                ...
                if ((flags & UC_PRESENT) != 0){
                    ...
                    read_UC_sparse(&(node->UC_array), file, root->info_per_lvl[(size_kmer/NB_CHAR_SUF_PREF)-1].size_kmer_in_bytes, nb_children >> 1);
                    node->UC_array.nb_children = nb_children;
                }
