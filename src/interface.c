...
*/
void insert_genomes_from_files(int nb_files, char** paths, BFT* bft){

    if (bft->marked == 0) insert_Genomes_from_KmerFiles(bft, nb_files, paths, 0);
    ...
    if (id_genome < bft->nb_genomes){
        ...
        if (id_genome < bft->nb_genomes/2){
            ...
            return is_genome_present(bft->ann_inf, bft->comp_set_colors, bft_annot->annot, bft_annot->size_annot,
                                     bft_annot->annot_ext, size_annot_ext, id_genome);
        }
...
* @param args should contain all additional arguments to pass to f. They can be extracted in f via its parameter of type va_list.
*/
...
*/
void write_BFT(BFT* bft, char* filename){

    ASSERT_NULL_PTR(bft, "write_BFT()\n")
    ASSERT_NULL_PTR(filename, "write_BFT()\n")

    write_BFT_Root_sparse(bft, filename);
...
BFT* load_BFT(char* filename_and_path){
    ...
    return read_BFT_Root_sparse(filename_and_path);
}
